...
OUTPUT_MD = ROOT / "top-25-departements-mal-inscrits-nupes-2022.md"
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Construit le tableau Top 25 mal-inscription x NUPES 2022."
    )
    parser.add_argument(
        "--refresh-insee",
        action="store_true",
...
def row_values(table, label):
    for row in re.findall(r"<tr[^>]*>(.*?)</tr>", table, flags=re.DOTALL):
        ...
        return [
            parse_number(value)
            for value in re.findall(r'<td[^>]*class="[^"]*nombre[^"]*"[^>]*>(.*?)</td>', row, re.DOTALL)
        ]
...
def fetch_insee(code):
    ...
    school_18_24, population_18_24 = row_values(
        education_table, "De 18 à 24 ans"
    )
    school_25_29, population_25_29 = row_values(
        education_table, "De 25 à 29 ans"
    )
...
def localized_young_estimate(row):
    ...
    # La classe Insee 25-29 est répartie uniformément : 1/5 à 25 ans,
    # puis 4/5 de 26 à 29 ans.
...
def build_markdown(top25, demography):
    ...
    for rank, row in enumerate(top25, start=1):
        ...
        population_18_29 = int(age["population_18_24"]) + int(
            age["population_25_29"]
        )
        enriched.append(
            {
                "rank": rank,
    ...
    lines = [
        "# Top 25 des départements mal-inscription x vote NUPES en 2022",
        "",
        "Classement selon la moyenne géométrique des rangs percentiles de "
        "mal-inscription et de vote NUPES :",
        "",
        ...
        "",
        "| Rang | Département | Mal-inscrits estimés | 18-34 ans estimés, méthode territorialisée | 18-34 ans estimés, ancienne méthode uniforme 52 % | Population scolarisée 18-29 ans | Part scolarisée des 18-29 ans | Mal-inscrits | Vote NUPES | Score croisé |",
        "|---:|---|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
    ...
    lines.extend(
        [
            ...
            "",
            "`part départementale Insee × inscrits électoraux aux législatives 2022`.",
            "",
            ...
            "```",
            "",
            "Le taux de 35,18 % répartit uniformément la classe 25-29 ans : "
            "un cinquième à 38,7 % pour les 25 ans et quatre cinquièmes à "
            "34,3 % pour les 26-29 ans. La population 30-34 ans est approchée "
            "par un tiers de la classe 30-44 ans.",
            "",
            ...
            "",
            "`mal-inscrits estimés × 0,52`.",
            "",
            "Cette colonne est conservée uniquement pour permettre la comparaison "
            "avec l'ancienne méthode. Multiplier chaque département par la même "
            "constante ne crée aucune information territoriale supplémentaire.",
            "",
            "### Population scolarisée",
            "",
            "L'indicateur additionne les populations scolarisées de 18-24 ans et "
            "de 25-29 ans du tableau FOR T1 de l'Insee. Il s'agit d'un indicateur "
            "séparé de présence étudiante ou scolaire, pas d'une estimation du "
            "nombre d'étudiants mal inscrits.",
            "",
            "Sources : Insee Première n°1986, figures 3 et 4 ; Insee RP2022, "
            "dossiers complets départementaux, tableaux POP T0 et FOR T1 ; "
            "ministère de l'Intérieur, législatives 2022.",
            "",
            "La part NUPES totale n'est pas calculée : les pourcentages "
            "départementaux de coalition proviennent de Wikipédia, tandis que "
            "les exprimés disponibles dans le pipeline proviennent du ministère. "
            "Les mélanger recréerait un ratio hybride sans définition homogène.",
            "",
        ]
    )
